...
V3D_RESULT V3DResourceMemory::Initialize(IV3DDevice* pDevice, V3DFlags propertyFlags, uint32_t resourceCount, IV3DResource** ppResources, const wchar_t* pDebugName)
{
	...
	for (uint32_t i = 0; i < resourceCount; i++)
	{
		...
		case V3D_RESOURCE_TYPE_BUFFER:
			...
			{
				V3D_LOG_ERROR(Log_Error_AlreadyBindResourceMemory, V3D_SAFE_NAME(this, pDebugName), V3D_LOG_TYPE(ppResources), i, static_cast<V3DBuffer*>(ppResources[i])->GetDebugName());
		...
		case V3D_RESOURCE_TYPE_IMAGE:
			...
			{
				V3D_LOG_ERROR(Log_Error_AlreadyBindResourceMemory, V3D_SAFE_NAME(this, pDebugName), V3D_LOG_TYPE(ppResources), i, static_cast<IV3DImageBase*>(ppResources[i])->GetDebugName());
				return V3D_ERROR_FAIL;
			}
			break;
		}
