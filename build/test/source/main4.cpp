...
protected:
	virtual bool OnInitialize() override
	{
		...
		m_Font.Initialize(Application::GetDevice(), GetWorkQueue(), GetWorkCommandBuffer(), GetWorkFence(), L"Arial", 16, 400);
	...
	{
		// テキストを更新
		m_Text = L"Fps " + std::to_wstring(Application::GetFps());

		m_Font.Reset();
		m_Font.AddText(16, 16, m_Text.c_str());
		m_Font.Bake();
...
//		desc.layer = V3D_LAYER_RENDERDOC;
	...
	virtual bool OnInitialize() override
	{
		...
		IV3DQueue* pQueue;
		Application::GetDevice()->GetQueue(queueFamily, 0, &pQueue);

		if (m_Window.Initialize(L"textureCube", 1024, 768, false, WINDOW_BUFFERING_TYPE_FAKE, pQueue, pQueue) == false)
		{
			SAFE_RELEASE(pQueue);
			return false;
		}
