...
protected:
	virtual bool OnInitialize() override
	{
...

		{
			...
			CreateFilePath(L"shader\\source\\texture3d.vert", srcFilePath);
			CreateFilePath(L"shader\\texture3d.vert.spv", dstFilePath);
			...
			CreateFilePath(L"shader\\source\\texture3d.frag", srcFilePath);
			CreateFilePath(L"shader\\texture3d.frag.spv", dstFilePath);
			if (CompileShaderFromFile(COMPILE_SHADER_SPIR_V_BINARY_VULKAN, srcFilePath.c_str(), dstFilePath.c_str()) != V3D_OK)
			{
				return false;
			}
		...
		{
			Array1<V3DConstantDesc, 2> constants =
			{
				{
					{ V3D_SHADER_STAGE_VERTEX, 0, sizeof(Matrix4x4) },
					{ V3D_SHADER_STAGE_FRAGMENT, 64, sizeof(float) }
				}
			};
...

		{
			...
			CreateFilePath(L"image\\texture3d.dds", filePath);
...

		{
			...
			samplerDesc.magFilter = V3D_FILTER_LINEAR; // 線形保管
	...
	virtual bool OnIdle() override
	{
		...
		m_Depth += TO_F32(Application::GetDeltaTime()) * FLOAT_RECIPROCAL(4.0f);
		if (m_Depth > 1.0f)
		{
			m_Depth -= 1.0f;
		}
...
class Texture3dApp : public Application
{
public:
	virtual ~Texture3dApp() {}
	...
	virtual bool OnInitialize() override
	{
		...
		if (m_Window.Initialize(L"texture3d", 1024, 768, true, WINDOW_BUFFERING_TYPE_FAKE, pWorkQueue, pGraphicsQueue) == false)
		{
